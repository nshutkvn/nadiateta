  ...
  if (img_container.classList.contains('hide_img')) {
    ...
    for (const file of files) {
      if (file.type.startsWith('image/')) {
        ...
        video.style.display = 'none'; // Initially hide all videos
        img_container.appendChild(video);
      }
    }

    // Display the first image/video
    img_container.children[currentImageIndex].style.display = 'block';
    ...
    nextButton.addEventListener('click', () => {
      ...
      for (let i = 0; i < files.length; i++) {
        img_container.children[i].style.display = 'none';
      }
      img_container.children[currentImageIndex].style.display = 'block';
    });
    ...
    img_container.appendChild(prevButton);
    img_container.appendChild(nextButton);

    // ... rest of your handleSubmit code, including next_btn_post, title_create, etc.
...
const descriptionInput = document.getElementById('emoji_create'); // Assuming this is the ID of your description textarea

form.addEventListener('submit', async (event) => { // Make the callback async
  event.preventDefault();
  ...
  for (const image of images) {
      ...
      formData.append('media', blob); // Use 'media' to match your backend key
  }

  try {
      const response = await fetch('create_post', { // Replace with your actual API endpoint URL
          method: 'POST',
          body: formData,
      });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  searchInput.addEventListener('input', function() {
    const query = searchInput.value.trim();
      if (query.length > 0) {
          // Check if the query is for mentors
          const isMentorSearch = query.toLowerCase().startsWith('mentor:');
          const cleanQuery = isMentorSearch ? query.substring(7).trim() : query; // Remove "mentor:" part if present

          // Make the AJAX request
          fetch(`/search/users/?query=${cleanQuery}&is_mentor=${isMentorSearch}`)
              ...
              .then(data => {
                  ...
                  if (data.users.length > 0) {
                      data.users.forEach(user => {
                          // Create a link for each user and display their info
                          const userLink = document.createElement('a');
                          userLink.href = `/profile/${user.username}/`; // Link to user's profile page

                          const userCart = document.createElement('div');
                          userCart.classList.add('cart');

                          const imgDiv = document.createElement('div');
                          imgDiv.classList.add('img');
                          const img = document.createElement('img');
                          img.src = user.profile_pic || '/path/to/default/image.png'; // Default image if no profile pic
                          ...
                          usernamePara.classList.add('name');
                          usernamePara.textContent = `@${user.username}`;

                          infoDiv.appendChild(namePara);
                          infoDiv.appendChild(usernamePara);

                          userCart.appendChild(imgDiv);
                          userCart.appendChild(infoDiv);
                          userLink.appendChild(userCart);
          ...
          resultsContainer.innerHTML = ''; // Clear results when the input is empty
      }
  });
});
